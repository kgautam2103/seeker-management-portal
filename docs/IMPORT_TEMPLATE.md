...
**Status:** v1.2 (KG decisions applied 2026-09-22) · **File:** [`assets/seeker-import-template.csv`](assets/seeker-import-template.csv) (header row + two synthetic example rows) · **Format:** CSV or XLSX, UTF-8, one seeker per row, first row = headers exactly as below.
...
| 3 | `phone` | **Either email or phone** | Any readable format; normalized to E.164 using `country` | `seeker.phone_e164` | Secondary dedupe key. A phone that cannot be normalized sends the row to review. |
| 4 | `country` | Recommended | Country name **or** ISO 3166-1 alpha-2 code (`United States` or `US`, `India` or `IN`) | `seeker.country_code`, phone normalization | Normalized to the ISO-2 code. Blank → the center's country if a center is given, else the batch's default country. A name that cannot be resolved sends the row to review. |
...
| `needs_review` | Email matches one seeker and phone matches a different one; name very similar to an existing seeker with different contact; phone cannot be normalized; `country` cannot be resolved; `center` given but matches no center | Row held in the review queue with the candidate matches or the problem shown |
...
- Whitespace is trimmed; emails are lower-cased; phones are normalized to E.164; country names are normalized to ISO-2 codes.
